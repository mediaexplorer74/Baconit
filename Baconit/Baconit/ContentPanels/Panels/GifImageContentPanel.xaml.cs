        ...
        private async Task<string> ConvertGifUsingGfycat(string gifUrl)
        {
            ...
            try
            {
                ...
                using (JsonReader jsonReader = new JsonTextReader(reader))
                {
                    // Parse the Json as an object
                    JsonSerializer serializer = new JsonSerializer();
                    GfyCatConversionData gfyData = await Task.Run(() =>
                    {

                        GfyCatConversionData rslt = default;

                        try
                        {
                            rslt = serializer.Deserialize<GfyCatConversionData>(jsonReader);
                        }
                        catch (Exception ex)
                        {
                            Debug.WriteLine("[ex] JSON Deserialize Exception : " + ex.Message);
                        }
